...
한 줄만 쓰면 정중한 비즈니스 메일로 바꿔주는 웹 (React + Vite)
...
실제 Claude API를 연결하려면:

1. `.env.example`을 복사해 `.env.local` 파일을 만듭니다.
2. [Anthropic Console](https://console.anthropic.com/)에서 발급받은 API 키를
   `.env.local`의 `VITE_CLAUDE_API_KEY`에 입력합니다.
3. `src/lib/convert.js`에서 `USE_MOCK`을 `false`로 바꿉니다.
4. 개발 서버를 다시 시작합니다.

## 보안 주의사항

- 이 앱은 백엔드 없이 **브라우저에서 직접** Claude API를 호출합니다.
